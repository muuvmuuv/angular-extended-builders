...
export async function loadPlugins(
	...
	const pluginPromises = paths.map(async (pluginOrPath): Promise<PluginLoadResult> => {
		debug.debug(`Starting load: ${pluginOrPath}`)
		try {
			const startTime = performance.now()
			const plugin = await loadModule<Plugin>(workspaceRoot, pluginOrPath, tsConfig)
	...
	// Report results
	if (failed.length > 0) {
		debug.warn(`Failed to load ${failed.length} plugin(s): ${failed.join(", ")}`)
	}
