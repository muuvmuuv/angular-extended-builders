...
function executeBuilder(options: ExtendedApplicationBuilderOptions, context: BuilderContext) {
	...
	return defer(async (): Promise<ApplicationBuilderExtensions> => {
		...
		const projectMetadata = projectName ? await context.getProjectMetadata(projectName) : null
		...
		const codePlugins = await loadPlugins(options.plugins, projectRoot, tsConfig)
		...
		if (options.indexHtmlTransformer) {
			...
			try {
				...
				debug.error(`Failed to load HTML transformer: ${options.indexHtmlTransformer}`, error)
				throw error
			}
		}

		const setupDuration = performance.now() - setupStartTime
		debug.debug(`Application builder setup completed in ${setupDuration.toFixed(2)}ms`)
